...
# render_250_test.sh
#
# Renders frames 1500-1750 (251 frames) for all 4 cameras into a fresh
# synth_dataset_test/ directory.  Splits into 4 parallel Blender instances.
#
...
OUT_DIR="$BASE/synth_dataset_test"
LOG_DIR="$BASE/render_logs_test"
...
# ── Split 1500-1750 into N batches ────────────────────────────────────────────
TOTAL=$(( END - START + 1 ))
BATCH=$(( (TOTAL + N - 1) / N ))
...
for i in $(seq 0 $(( N - 1 ))); do
    ...
    [ "$BE" -gt "$END" ] && BE=$END
    [ "$BS" -gt "$END" ] && break
    FILE="$LOG_DIR/batch_test_$(printf '%02d' $i).txt"
    seq "$BS" "$BE" > "$FILE"
    BATCH_FILES+=("$FILE")
...
echo "      Output -> $OUT_DIR"
echo "      Logs   -> $LOG_DIR/render_test_*.log"
echo ""
...
for i in "${!BATCH_FILES[@]}"; do
    LOG="$LOG_DIR/render_test_$(printf '%02d' $i).log"
...
for i in "${!PIDS[@]}"; do
    if wait "${PIDS[$i]}"; then
        ...
        echo "   Batch $i FAILED. Check $LOG_DIR/render_test_$(printf '%02d' $i).log"
        ALL_OK=false
    fi
done
...
# ── Flatten per-camera outputs into synth_dataset_test/images + labels ────────
echo ""
...
relabel_synth.LABELS_DIR  = Path("/Users/awthura/OVGU/AMS/synthetic_polybags/synth_dataset_test/labels")
relabel_synth.IMAGES_DIR  = Path("/Users/awthura/OVGU/AMS/synthetic_polybags/synth_dataset_test/images")
relabel_synth.CLASSES_TXT = Path("/Users/awthura/OVGU/AMS/synthetic_polybags/synth_dataset_test/classes.txt")
relabel_synth.main()
PYEOF
...
BASE    = Path("/Users/awthura/OVGU/AMS/synthetic_polybags")
MOT_DIR = BASE / "synth_dataset_test" / "mot_obb"
...
echo ""
echo "======================================================"
echo "  Test dataset done in $(( TOTAL_TIME / 60 )) min $(( TOTAL_TIME % 60 )) s"
