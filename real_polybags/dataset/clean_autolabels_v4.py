"""
Same cleanup pipeline as clean_autolabels.py, applied to the round-2
(real_v4_round2) autolabel run instead of the original raw run -- see
AUTOLABEL_REPORT.md section 8. Kept as a separate script (rather than
parametrizing clean_autolabels.py) since the two runs' outputs need to stay
independently inspectable for the before/after comparison.

Usage:
    python clean_autolabels_v4.py
"""
...
LABELS_ROOT = DATASET_ROOT / "unlabelled_autolabels_v4" / "labels"
OUT_ROOT = DATASET_ROOT / "unlabelled_autolabels_v4"
...
def main():
    ...
    print(f"Processing {len(label_files)} label files (real_v4_round2)...")
